...
def gust_simple(wind,tke,rng,cf=0.1):
    sigma = np.sqrt(tke * (2.0/3.0)) * cf       # std of turbulent kinetic energy with correction factor cf
    eta = rng.normal(0.0, 1.0, size=3)          # draw three random numbers     
    gust = sigma * eta                          # gaussian distributed gust components u',v',w'
    wind_new = wind + gust                      # effective wind u,v,w = mean u,v,w + gust u',v',w'
    return wind_new

def gust_OU(wind, tke, gust_old, rng, dt=0.01, Tg=0.3, cf=0.1):                 # Ornstein-Uhlenbeck inspired model
    sigma = np.sqrt(tke * (2.0/3.0)) * cf                                       # std of turbulent kinetic energy with correction factor cf
    eta = rng.normal(0.0, 1.0, size=3)                                          # draw three random numbers
    gust_new = gust_old*(1.0 - dt/Tg) + np.sqrt(2.0*sigma**2*dt/Tg) * eta       # new gust = decaying previous gust term + new random component
    wind_new = wind + gust_new                                                  # effective wind u,v,w = mean u,v,w + gust u',v',w'
